...
class Tela {
  ...
  static obterPerguntaHTML(item)
  {
    return `    
    <div style="display: flex; justify-content: center;">
      <div id=${item.id} class="card" style="width: 250px;">
        <img src="${item.img}" name="${item.nome}" class="card-img-top" alt="...">
      </div>
    </div>
  `
  ...
  static alterarPerguntaHTML(codigoHTML) {
    const conteudo = document.getElementById('pergunta')
    conteudo.innerHTML = codigoHTML
  }
  ...
  static atualizarPergunta(item){
    const codigoHTML = Tela.obterPerguntaHTML(item)
    Tela.alterarPerguntaHTML(codigoHTML)
  }
  ...
  // static exibirPontuacao(pontuacao) {
  //   const elemento = document.getElementById(ID_PONTUACAO)
  //   elemento.innerText = pontuacao
  // }
}